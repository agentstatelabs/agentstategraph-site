...
(function () {
  ...
  window.selectPath = function (path) {
    ...
    if (blame) {
      ...
      if (blame.commit_id) html += detailRow('Commit', `<code>${blame.commit_id}</code>`);
      html += detailRow('Agent', blame.agent_id);
      html += detailRow('Intent', `${blame.intent_category} — ${blame.intent_description}`);
      if (blame.intent_status) html += detailRow('Status', blame.intent_status);
      if (blame.intent_tags && blame.intent_tags.length) {
        html += detailRow('Tags', blame.intent_tags.map(t => `<span class="pill pill-category">${t}</span>`).join(''));
      }
      ...
      if (blame.tool_calls && blame.tool_calls.length) {
        html += detailRow('Tool calls', blame.tool_calls.map(t => `<code>${t}</code>`).join(', '));
      }
      html += `</div>`;
    }
  ...
  function renderTimeline() {
    ...
    el.innerHTML = DEMO.commits.map(c => {
      ...
      return `<div class="tl-entry">` +
        ...
        (c.authority ? detailRow('Authority', c.authority) : '') +
        (c.tags && c.tags.length ? detailRow('Tags', c.tags.map(t => `<span class="pill pill-category">${t}</span>`).join('')) : '') +
